...
COMPOSE_FILE="${COMPOSE_FILE:-docker-compose.vps2.yml}"
DB_SERVICE="${DB_SERVICE:-postgres}"
DB_NAME="${DB_NAME:-trading}"
DB_USER="${DB_USER:-postgres}"

command -v docker >/dev/null 2>&1 || {
  echo "docker is required to apply VPS2 migrations" >&2
  exit 1
}

docker compose version >/dev/null 2>&1 || {
  echo "docker compose is required to apply VPS2 migrations" >&2
  exit 1
}
