...
from .phase_scoring import nulrimok_reject_reason, score_nulrimok_phase
...
def init_worker(config: dict[str, Any] | None, phase: int, hard_rejects: dict[str, float] | None, scoring_weights: dict[str, float] | None) -> None:
    ...
    _worker_replay_bundle = (
        make_synthetic_replay_bundle("nulrimok", _worker_config)
...
def score_candidate(args) -> ScoredCandidate:
    ...
    result = run_nulrimok_backtest(_worker_config, mutations, replay_bundle=_worker_replay_bundle)
    metrics = result.metrics
    reject = nulrimok_reject_reason(_worker_phase, metrics, _worker_hard_rejects)
    if reject:
        return ScoredCandidate(name=name, score=0.0, rejected=True, reject_reason=reject, metrics=metrics)
    return ScoredCandidate(name=name, score=score_nulrimok_phase(_worker_phase, metrics, _worker_scoring_weights), metrics=metrics)
