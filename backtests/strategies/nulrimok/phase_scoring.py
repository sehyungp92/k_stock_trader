...
PHASE_HARD_REJECTS = {phase: {"min_trades": 1, "max_dd_pct": 0.15, "max_same_bar_fills": 0} for phase in range(1, 7)}
PHASE_SCORING_WEIGHTS = {
    phase: {"expectancy": 0.25, "profit_factor": 0.25, "expected_total_r": 0.20, "mfe_capture": 0.15, "max_drawdown_pct": -0.15}
    for phase in range(1, 7)
}


def score_nulrimok_phase(phase: int, metrics: dict[str, float], weights: dict[str, float] | None = None) -> float:
    return score_metrics(metrics, weights or PHASE_SCORING_WEIGHTS.get(phase))


def nulrimok_reject_reason(phase: int, metrics: dict[str, float], hard_rejects: dict[str, float] | None = None) -> str:
